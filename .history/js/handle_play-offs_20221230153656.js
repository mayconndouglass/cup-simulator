...
export default function HandlePlayOffs({
  ...
  const handleFinals = event => {
    ...
    const checkMatchBank = checkMatchInTheBank(qualifierMatchBank[3], confrontation)

    if (checkMatchBank !== -1) {
      const indexElement = searchIndex(confrontation, qualifierBank[3])

      removeConfrontationFromTheBank(qualifierMatchBank[3], checkMatchBank)
      removeConfrontationFromTheBank(qualifierBank[3], indexElement)
    }
    ...
    removeOrAddClassHidden(fieldsPenalties, 1)
    updateQualifyingBank(finalBank, qualifierMatchBank)
  }
