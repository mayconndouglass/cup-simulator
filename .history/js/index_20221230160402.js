...
const loadPlayoffData = (dataSet, classs, numberBank) => {
  ...
  bank[numberBank].forEach(({ penalty, goals }) => {
    const fields = document.querySelector(`.${classs} [${dataSet}]`)

    if (penalty) {
      const fieldsPenalties = document.querySelector(`.${classs} [data-penalty]`)
      utilities.removeOrAddClassHidden(fieldsPenalties, 1)

      fieldsPenalties.forEach((field, index) => {
        field.value = penalty[index]
      })
      const [fieldTeamOne, fieldTeamTwo] = fieldsPenalties
      [fieldTeamOne.value, fieldTeamTwo.value] = penalty
      fieldTeamOne.value = penalty[0]
      fieldTeamTwo.value = penalty[1]
      return
    }
  })
}
